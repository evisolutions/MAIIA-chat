<?php
    ...
    function mojo_enqueue_chat_widget() {
        ...
        // Get options
        $domain = get_option('mojo_domain', 'https://websiteaddon.maiiaconcierge.ai');
        $property_id = get_option('mojo_property_id', '19');
        $conversation_type = get_option('mojo_conversation_type', 'support');
        $z_index = get_option('mojo_z_index', '9999');

        // Mojo Widget Styles & Script
        wp_enqueue_style('mojo-widget', "{$domain}/style.css");
        wp_enqueue_script('mojo-widget', "{$domain}/maiia-widget.umd.js");
        ?>
        <script>
    ...
    function mojo_register_settings() {
        ...
        add_option('mojo_property_id', '19');
